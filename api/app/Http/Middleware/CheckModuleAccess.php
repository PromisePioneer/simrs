...
class CheckModuleAccess
{
    ...
    public function handle(Request $request, Closure $next, string $moduleName): Response
    {
        ...
        $module = Cache::remember(
            ...
            now()->addHour(),
            fn() => Module::where('name', $moduleName)
                ->with('permissions')
                ->first()
        );
        ...
        // Gunakan getActivePermissions() dari HasActiveTenant trait
        // supaya session active_role_id tetap dihormati
        $modulePermissionNames = $module->permissions->pluck('name')->toArray();

        // Kalau module tidak punya permission terdaftar, allow saja
        // (artinya module ini tidak butuh permission spesifik)
