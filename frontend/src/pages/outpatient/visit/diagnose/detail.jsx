...
import {
    ...
    Weight, AlertCircle, IdCard, BookOpen, UserCheck
} from "lucide-react";
...
function SectionTitle({icon: Icon, title, accent = "teal", count}) {
    ...
    return (
        <div className="flex items-center gap-2.5">
            ...
            {count !== undefined && (
                <span className="ml-auto text-xs font-medium text-slate-400 bg-slate-100 rounded-full px-2 py-0.5">{count}</span>
            )}
        </div>
    );
}
...
function Panel({children, className}) {
    return <div className={cn("bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden", className)}>{children}</div>;
}
...
function OutpatientVisitDetail(opts) {
    ...
    useEffect(() => { showOutPatientVisit(id); }, [id]);
        ...
        <div className="space-y-5 max-w-screen-xl mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
                    ...
                    <PanelBody className="space-y-4">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="ml-auto">
                                <span className="text-sm font-bold px-3 py-1 rounded-lg bg-red-50 text-red-600 border border-red-100">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                            ...
                            <Field label="Tanggal Lahir" value={patient?.date_of_birth ? format(new Date(patient.date_of_birth), "dd MMM yyyy", {locale: localeId}) : null}/>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="flex items-center gap-2.5">
                                ...
                                <div>
                                    <p className="text-[11px] uppercase tracking-wide font-medium text-slate-400">No. KTP</p>
                ...
                <div className="lg:col-span-2 flex flex-col gap-5">
                    <Panel>
                        <PanelHeader><SectionTitle icon={Stethoscope} title="Dokter Pemeriksa" accent="violet"/></PanelHeader>
                            ...
                            {(visit.referred_hospital || visit.referred_doctor) && (
                                <>
                                    ...
                                    <div className="space-y-2">
                                        {visit.referred_hospital && <Field label="RS Perujuk"      value={visit.referred_hospital}/>}
                                        {visit.referred_doctor   && <Field label="Dokter Perujuk"  value={visit.referred_doctor}/>}
                                    </div>
                                </>
                            )}
                        </PanelBody>
                    ...
                    <Panel className="flex-1">
                        <PanelHeader><SectionTitle icon={AlertCircle} title="Keluhan Utama" accent="rose"/></PanelHeader>
                        <PanelBody>
                            <p className="text-sm text-slate-700 leading-relaxed">
                                {visit.complain || <span className="text-slate-400 italic">Tidak ada keluhan tercatat</span>}
                            </p>
                        </PanelBody>
                    </Panel>
                </div>
            ...
            {vital_sign && (
                <Panel>
                    <PanelHeader><SectionTitle icon={Activity} title="Tanda-Tanda Vital" accent="sky"/></PanelHeader>
                    <PanelBody>
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                            <VitalCard icon={Ruler}       label="Tinggi Badan"     value={vital_sign.height}                  unit="cm"    color="slate"/>
                            <VitalCard icon={Weight}      label="Berat Badan"      value={vital_sign.weight}                  unit="kg"    color="slate"/>
                            <VitalCard icon={Thermometer} label="Suhu Tubuh"       value={vital_sign.temperature}             unit="°C"    color="amber"/>
                            <VitalCard icon={Heart}       label="Nadi"             value={vital_sign.pulse_rate}              unit="bpm"   color="rose"/>
                            <VitalCard icon={Wind}        label="Frekuensi Napas"  value={vital_sign.respiratory_frequency}   unit="x/mnt" color="sky"/>
                            <VitalCard icon={Gauge}       label="Sistol"           value={vital_sign.systolic}                unit="mmHg"  color="violet"/>
                            <VitalCard icon={Gauge}       label="Diastol"          value={vital_sign.diastolic}               unit="mmHg"  color="violet"/>
                            <VitalCard icon={Droplets}    label="Gula Darah"       value={vital_sign.blood_sugar}             unit="mg/dL" color="teal"/>
                            <VitalCard icon={Activity}    label="Saturasi O₂"      value={vital_sign.oxygen_saturation}       unit="%"     color="teal"/>
                            <VitalCard icon={Ruler}       label="Lingkar Perut"    value={vital_sign.abdominal_circumference} unit="cm"    color="slate"/>
                        </div>
                    </PanelBody>
                </Panel>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <Panel>
                    <PanelHeader><SectionTitle icon={BookOpen} title="Diagnosa" accent="teal" count={diagnoses.length}/></PanelHeader>
                    <PanelBody>
                        {diagnoses.length > 0 ? (
                            <div className="space-y-2">
                                {diagnoses.map((d, i) => (
                                    <div key={i} className="flex items-start gap-3 p-3 bg-slate-50 rounded-xl">
                                        <div className="flex flex-col gap-1 shrink-0">
                                            <span className="text-xs font-bold font-mono bg-teal-100 text-teal-700 px-2 py-0.5 rounded-lg text-center">{d.icd10_code}</span>
                                            <span className={cn("text-[10px] font-semibold px-1.5 py-0.5 rounded-full text-center",
                ...
                <Panel>
                    <PanelHeader><SectionTitle icon={ClipboardList} title="Prosedur" accent="violet" count={procedures.length}/></PanelHeader>
                    <PanelBody>
                        {procedures.length > 0 ? (
                            <div className="space-y-2">
                                {procedures.map((p, i) => (
                                    <div key={i} className="flex items-start gap-3 p-3 bg-slate-50 rounded-xl">
                                        <span className="text-xs font-bold font-mono bg-violet-100 text-violet-700 px-2 py-0.5 rounded-lg shrink-0 mt-0.5">{p.icd9_code}</span>
            ...
            {/* Row 4: Resep */}
            <Panel>
                <PanelHeader><SectionTitle icon={Pill} title="Resep Obat" accent="amber" count={prescriptions.length}/></PanelHeader>
                            ...
                            <table className="w-full text-sm">
                                ...
                                <tr className="bg-slate-50 border-b border-slate-100">
                                    {["Obat","Dosis","Frekuensi","Rute","Durasi","Qty","Catatan","Status"].map(h => (
                                        <th key={h} className="text-left px-5 py-3 text-[11px] uppercase tracking-wide font-semibold text-slate-400">{h}</th>
                                    ))}
                                </tr>
                                </thead>
