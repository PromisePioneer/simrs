...
const planCardVariants = cva("relative border transition-all duration-300 rounded-lg", {
  variants: {
    size: {
      small: "p-4",
      medium: "p-5",
...
const contactCardVariants = cva("border transition-all duration-300 h-full rounded-lg", {
  variants: {
    size: {
      small: "p-6",
      medium: "p-7",
    ...
    <section className={cn(sectionVariants({ size, theme }), className)}>
      ...
      {theme === "classic" && (
        <>
          ...
          <div
            className="absolute top-1/4 right-1/4 w-64 h-64 bg-secondary/5 rounded-full blur-2xl" />
        </>
      )}
      <div className="container max-w-7xl mx-auto relative">
        ...
        <div className="text-center mb-12">
          ...
          {/* Billing Toggle */}
          <div
            className={cn("flex justify-center mt-8 mx-auto", toggleVariants({ theme }))}>
            <RadioGroup
              ...
              <div
                ...
                <Label
                  ...
                  {yearlyPriceDiscount > 0 && (
                    <span
                      className="ml-1 rounded bg-primary/10 px-2 py-0.5 text-xs text-primary border border-primary/20 font-medium">
          ...
          <div className="flex flex-col gap-4 lg:w-2/3">
            {regularPlans.map((plan, index) => (
              <motion.div
                ...
                <Card
                  className={cn(planCardVariants({ size, theme, highlight: plan.highlight }))}>
                  {plan.badge && (
                    <Badge
                      className={cn(
                  ...
                  {theme === "classic" && plan.highlight && (
                    <div
                      className="absolute -top-px left-1/2 -translate-x-1/2 w-32 h-px bg-gradient-to-r from-transparent via-primary to-transparent" />
                  )}

                  <CardContent
                    className="p-0 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                    {/* Plan Info and Price */}
                    <div className="flex flex-col gap-3 min-w-[200px]">
                      <Badge variant="outline" className="w-fit text-xs font-medium uppercase">
                        {plan.title}
                      </Badge>
